...
import com.gientech.iot.user.biz.constant.Constants;
...
import com.gientech.iot.user.biz.mapper.DepartmentMapper;
import com.gientech.iot.user.biz.service.DepartmentService;
import com.gientech.iot.user.biz.service.DepartmentUserService;
...
public class DepartmentServiceImpl extends ServiceImpl<DepartmentMapper, SysDepartment> implements DepartmentService {
    ...
    public String getNo() {
        ...
        if (val.length() == 0) {
            ...
            try {
                ...
                throw new GientechException(CustomErrorCodeEnum.CODE_CACHE_ERROR);
            } finally {
                // 如果加锁成功则进行解锁
                if (lock) {
    ...
    public DepartmentCreateResultVO create(DepartmentCreateDTO dto) {
        ...
        if (parent == null) {
            throw new GientechException(CustomErrorCodeEnum.PARENT_NOT_EXIT);
        }
        // 部门编码生成前缀
        String prefix = Constants.CodeRule.DEPT_CODE_PREFIX;
        // 部门可用编码对应的redis中的key
        ...
        try {
            if (lock) {
                ...
                }).orElseThrow(() -> new GientechException(CustomErrorCodeEnum.DEPARTMENT_CREATE_ERROR));
            ...
            throw new GientechException(CustomErrorCodeEnum.DEPARTMENT_CREATE_ERROR);
        } finally {
            // 如果加锁成功则进行解锁
            if (lock) {
                redisUtils.tryReentrantUnlock(lockKey, "");
    ...
    public void update(DepartmentUpdateDTO dto) {
        ...
        if (StringUtils.isEmpty(department.getIds()) || !Objects.equals(department.getParentId(), dto.getParentId())) {
            ...
            SysDepartment dept = BeanConvertUtils.convert(dto, SysDepartment::new, (s, t) -> {
                ...
                t.setIds(parent.getIds() == null ? String.valueOf(t.getId()) : parent.getIds() + "," + t.getId());
                t.setLevel(t.getIds().split(",").length);
            }).orElseThrow(() -> new GientechException(CustomErrorCodeEnum.DEPARTMENT_UPDATE_ERROR));
            // 获取部门下的子部门列表
    ...
    public void delete(List<Long> departmentIds) {
        ...
        if (CollectionUtils.isNotEmpty(departmentUserService.getUserIdsByDeptIds(deptIdList))) {
            throw new GientechException(CustomErrorCodeEnum.DEPARTMENT_HAS_USER);
        }
