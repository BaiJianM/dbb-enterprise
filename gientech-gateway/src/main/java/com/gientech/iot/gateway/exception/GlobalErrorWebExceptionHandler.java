package com.gientech.iot.gateway.exception;
...
public class GlobalErrorWebExceptionHandler implements ErrorWebExceptionHandler {
    ...
    public Mono<Void> handle(ServerWebExchange exchange, @NonNull Throwable throwable) {
        ...
        // 自定义异常处理
        if (throwable instanceof GientechException) {
            GientechException exception = (GientechException) throwable;
            return response.writeWith(Mono.fromSupplier(() -> {
                DataBufferFactory bufferFactory = response.bufferFactory();
                try {
                    return bufferFactory.wrap(objectMapper.writeValueAsBytes(GientechResponse.fail("", exception).getBody()));
        ...
        return response.writeWith(Mono.fromSupplier(() -> {
            ...
            try {
                return bufferFactory.wrap(
                        objectMapper.writeValueAsBytes(GientechResponse.fail(throwable.getMessage()).getBody()));
            } catch (JsonProcessingException e) {
                log.error("网关全局异常拦截响应异常", throwable);
                return bufferFactory.wrap(new byte[0]);
            }
