...
import com.gientech.iot.thread.GientechThreadPool;
...
import static com.gientech.iot.gateway.constant.GatewayConstants.AuthConstant.ACCOUNT_FREEZE_PREFIX;
import static com.gientech.iot.gateway.constant.GatewayConstants.AuthConstant.RETRY_KEY_PREFIX;
import static com.gientech.iot.gateway.constant.GatewayConstants.AuthErrorConstant.*;
...
public class AccountReactiveAuthenticationManager implements BaseAuthenticationManager<AccountAuthentication> {
    ...
    public Mono<Authentication> authenticate(AccountAuthentication authentication) {
        ...
        String freezeKey = String.format(ACCOUNT_FREEZE_PREFIX, user.getUserId());
        if (redisUtils.hasKey(freezeKey).orElse(false)) {
            ...
            String errorMsg = String.format(ACCOUNT_FREEZE, user.getRetryNum(), unFreezeTime);
            return Mono.error(new BadCredentialsException(errorMsg));
        }
        // 用户账号密码输错后的重试计数缓存
        String retryKey = String.format(RETRY_KEY_PREFIX, user.getUserId());
        // 校验密码
        if (!passwordEncoder.matches(password, user.getPassword())) {
            // 如果已经存在重试计数缓存，就取出其计数值，并判断是否需要返回冻结
            AtomicReference<String> notice = new AtomicReference<>(String.format(PASSWORD_ERROR, user.getRetryNum()));
                ...
                redisUtils.get(retryKey).ifPresent(val -> {
                    ...
                    if (count == 1) {
                        ...
                        String unFreezeTime = DateUtils.parseTime(
                                ...
                                DateUtils.TimeFormat.LONG_DATE_PATTERN_LINE);
                        notice.set(String.format(ACCOUNT_FREEZE, user.getRetryNum(), unFreezeTime));
                    } else {
                        // 否则减一后再次缓存
                        count--;
                        redisUtils.set(retryKey, count);
                        notice.set(String.format(PASSWORD_ERROR, count));
                    }
                });
            } else {
                // 初始化重试计数缓存
        ...
        // 判断用户是否禁用
            ...
            return Mono.error(new DisabledException(ACCOUNT_DISABLED));
        }
        // 判断用户是否锁定
        else if (!user.isAccountNonLocked()) {
            return Mono.error(new LockedException(ACCOUNT_LOCKED));
        }
        // 判断账号是否过期
        else if (!user.isAccountNonExpired()) {
            return Mono.error(new AccountExpiredException(ACCOUNT_EXPIRED));
        }
        // 判断凭证是否过期
        else if (!user.isCredentialsNonExpired()) {
            return Mono.error(new CredentialsExpiredException(LOGIN_EXPIRED));
        }
        // 判断是否有系统权限
        else if (CollectionUtils.isEmpty(user.getAuthorities())) {
            return Mono.error(new CredentialsExpiredException(SYSTEM_FORBIDDEN));
        }
        // 异步处理成功认证后可能存在的登录重试计数缓存
        GientechThreadPool.initThreadPool().execute(() -> {
