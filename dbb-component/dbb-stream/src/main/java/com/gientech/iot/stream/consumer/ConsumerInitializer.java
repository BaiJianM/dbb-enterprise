...
public class ConsumerInitializer implements ApplicationListener<ApplicationEvent> {
     ...
     * 消费者监听类集合
     */
    private Map<String, GientechConsumerListener> consumerListeners;
    ...
    public void onApplicationEvent(@NonNull ApplicationEvent event) {
        if (!initTag) {
            // 获取所有实现了自定义消费者接口的实现类
            Map<String, GientechConsumerListener> consumerListeners = applicationContext.getBeansOfType(GientechConsumerListener.class);
            // 校验这些实现类是否包含规定注解
            // 校验所有消费者监听的tag有没有重复
            Set<CheckTag> repeatTags = new HashSet<>();
            consumerListeners.forEach((key, value) -> {
                String className = value.getClass().getSimpleName();
                GientechMQListener annotation = value.getClass().getAnnotation(GientechMQListener.class);
                if (annotation == null) {
                    throw new RuntimeException(className +
                            "实现了GientechConsumerListener接口，但是类上没有标注@GientechMQListener注解");
                }
                Arrays.stream(annotation.tags()).forEach(t -> {
                    ...
                    if (!add) {
                        throw new RuntimeException(className +
                                "实现了GientechConsumerListener接口，但是tag存在重复定义。重复定义的类为"
     ...
     * @return: java.util.Map<java.lang.String, com.gientech.iot.stream.consumer.GientechConsumerListener>
     * @version: 1.0
     */
    public Map<String, GientechConsumerListener> getConsumerListeners() {
